...
func SubtractDecCoinsWithRounding(d1s sdk.DecCoins, d2s sdk.DecCoins) sdk.DecCoins {
	...
	for _, d2 := range d2s {
		...
		a2 := d2.Amount
		if a2.GT(a1) && a2.Sub(a1).LT(sdkmath.LegacyOneDec()) {
...
func (v AllianceValidator) ValidatorSharesWithDenom(denom string) sdkmath.LegacyDec {
...
func (v AllianceValidator) TotalDelegationSharesWithDenom(denom string) sdkmath.LegacyDec {
	return sdk.DecCoins(v.TotalDelegatorShares).AmountOf(denom)
}

func (v AllianceValidator) TotalTokensWithAsset(asset AllianceAsset) sdkmath.LegacyDec {
	shares := v.ValidatorSharesWithDenom(asset.Denom)
	dec := ConvertNewShareToDecToken(sdkmath.LegacyNewDecFromInt(asset.TotalTokens), asset.TotalValidatorShares, shares)
	return dec
}

func GetValidatorShares(asset AllianceAsset, token sdkmath.Int) sdkmath.LegacyDec {
	return ConvertNewTokenToShares(sdkmath.LegacyNewDecFromInt(asset.TotalTokens), asset.TotalValidatorShares, token)
}
