...
func TestDelegateThenTakeRateThenUndelegate(t *testing.T) {
	app, ctx, vals, dels := setupApp(t, 5, 2, sdk.NewCoins(sdk.NewCoin("test", sdkmath.NewInt(1000000000000000000))))
	err := app.AllianceKeeper.CreateAlliance(ctx, &types.MsgCreateAllianceProposal{
		...
		RewardWeightRange:    types.RewardWeightRange{Min: sdkmath.LegacyZeroDec(), Max: sdkmath.LegacyMustNewDecFromStr("0.1")},
		TakeRate:             sdkmath.LegacyMustNewDecFromStr("0.02"),
		RewardChangeRate:     sdkmath.LegacyMustNewDecFromStr("0.01"),
	...
	_, err = app.AllianceKeeper.Delegate(ctx, dels[0], val0, sdk.NewCoin("test", sdkmath.NewInt(100033333333333333)))
	require.NoError(t, err)

	val0, err = app.AllianceKeeper.GetAllianceValidator(ctx, vals[0])
	require.NoError(t, err)
	require.Equal(t, sdkmath.LegacyNewDec(100033333333333333), sdk.DecCoins(val0.TotalDelegatorShares).AmountOf("test"))
	...
	require.Equal(t, sdkmath.LegacyZeroDec(), sdk.DecCoins(val0.ValidatorShares).AmountOf("test"))

	_, err = app.AllianceKeeper.Delegate(ctx, dels[0], val0, sdk.NewCoin("test", sdkmath.NewInt(33333)))
	require.NoError(t, err)
...
func TestDelegatingASmallAmount(t *testing.T) {
	app, ctx, vals, dels := setupApp(t, 2, 3, sdk.NewCoins(
		sdk.NewCoin(allianceAsset1, sdkmath.NewInt(1000000000000000000)),
		sdk.NewCoin(allianceAsset2, sdkmath.NewInt(1000000000000000000)),
	))
	startTime := time.Now()
	...
	app.AllianceKeeper.InitGenesis(ctx, &types.GenesisState{
		...
		Assets: []types.AllianceAsset{
			types.NewAllianceAsset(allianceAsset1, sdkmath.LegacyNewDec(2), sdkmath.LegacyNewDec(0), sdkmath.LegacyNewDec(5), sdkmath.LegacyNewDec(0), ctx.BlockTime()),
			types.NewAllianceAsset(allianceAsset2, sdkmath.LegacyNewDec(10), sdkmath.LegacyNewDec(2), sdkmath.LegacyNewDec(12), sdkmath.LegacyMustNewDecFromStr("0.1"), ctx.BlockTime()),
		},
	})
	...
	_, err = app.AllianceKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(allianceAsset2, sdkmath.NewInt(100)))
	require.NoError(t, err)
	_, err = app.AllianceKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(allianceAsset2, sdkmath.NewInt(1000_000_000)))
	require.NoError(t, err)
	...
	_, err = app.AllianceKeeper.Undelegate(ctx, user1, val1, sdk.NewCoin(allianceAsset2, sdkmath.NewInt(1000_000_000)))
	require.Error(t, err)
	...
	// Delegate again
	_, err = app.AllianceKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(allianceAsset2, sdkmath.NewInt(500_000_000)))
	require.NoError(t, err)

	ctx = ctx.WithBlockTime(ctx.BlockTime().Add(time.Minute * 1)).WithBlockHeight(2)

	_, err = app.AllianceKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(allianceAsset2, sdkmath.NewInt(400_000_000)))
	require.NoError(t, err)
	...
	require.NoError(t, err)
	del = res.GetDelegation()
	require.True(t, del.GetBalance().Amount.LT(sdkmath.NewInt(900_000_000)), "%s should be less than %s", del.GetBalance().Amount, sdkmath.NewInt(1000_000_000))
...
func TestDelegateAndUndelegateWithSmallAmounts(t *testing.T) {
	app, ctx, vals, dels := setupApp(t, 5, 2, sdk.NewCoins(
		sdk.NewCoin(allianceAsset1, sdkmath.NewInt(2000_000_000_000_000_000)),
		sdk.NewCoin(allianceAsset2, sdkmath.NewInt(2000_000_000_000_000_000)),
	))
	startTime := time.Now()
	ctx = ctx.WithBlockTime(startTime).WithBlockHeight(1)
	params := types.DefaultParams()
	...
	// Delegate token with non-zero take_rate
	_, err = app.AllianceKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(allianceAsset2, sdkmath.NewInt(200)))
	require.NoError(t, err)
	_, err = app.AllianceKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(allianceAsset2, sdkmath.NewInt(1000_000_000_000_000_000)))
	require.NoError(t, err)
	...
	_, err = app.AllianceKeeper.Undelegate(ctx, user1, val1, sdk.NewCoin(allianceAsset2, sdkmath.NewInt(1000_000_000_000_000_000)))
	require.Error(t, err)
	require.Equal(t, del.Balance.Amount, sdkmath.NewInt(200))
...
func TestUnDelegatingSlightlyMoreCoin(t *testing.T) {
	...
	// Delegate token with non-zero take_rate
	_, err = app.AllianceKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(allianceAsset2, sdkmath.NewInt(5000)))
	require.NoError(t, err)
	_, err = app.AllianceKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(allianceAsset2, sdkmath.NewInt(1000_000_000)))
	require.NoError(t, err)
...
func TestDustValidatorSharesAfterUndelegationError(t *testing.T) {
	...
	app.AllianceKeeper.InitGenesis(ctx, &types.GenesisState{
		...
		Assets: []types.AllianceAsset{
			types.NewAllianceAsset(allianceAsset1, sdkmath.LegacyNewDec(2), sdkmath.LegacyNewDec(0), sdkmath.LegacyNewDec(5), sdkmath.LegacyNewDec(0), ctx.BlockTime()),
			types.NewAllianceAsset(allianceAsset2, sdkmath.LegacyMustNewDecFromStr("10"), sdkmath.LegacyNewDec(5), sdkmath.LegacyNewDec(0), sdkmath.LegacyMustNewDecFromStr("0.1"), ctx.BlockTime()),
		},
	})
	queryServer := keeper.NewQueryServerImpl(app.AllianceKeeper)
	...
	// Delegate token with non-zero take_rate
	_, err = app.AllianceKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(allianceAsset2, sdkmath.NewInt(1000)))
	require.NoError(t, err)
	_, err = app.AllianceKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(allianceAsset2, sdkmath.NewInt(0)))
	require.NoError(t, err)
